...
import {
  Collection,
  CollectionReference,
} from 'src/renderer/models/responses/Collection';
import { MediaItem } from 'src/renderer/models/responses';
...
const SavedCollectionItem: FC<SavedCollectionItemProps> = ({
  ...
  const handleCollectionItemSave =
    (id: string, sequence: number | null) => () => {
      if (sequence !== null) {
        ...
        if (item) {
          // display error modal
          console.log('Sequence number already used');
        } else {
          onSave(id, sequence);
          setToggleEdit(false);
        }
  ...
  return (
    <div className={styles.savedCollectionListItem}>
      ...
      <div></div>
      <div className={styles.savedCollectionName}>{collection.title}</div>
      <div className={styles.savedCollectioneditable}>
        <div className={styles.sequenceNumber}>SEQ #</div>
        {toggleEdit ? (
          <div>{collectionRef.sequence}</div>
        ) : (
          <input
